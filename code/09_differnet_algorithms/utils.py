...
def genTokenValues(UB1 = 1, UB2 = 1, UB3 = 1, UB4 = 1, numBuyers=4, numSellers=4, numTokens=4, seed = None):
    if seed != None:
        np.random.seed(seed)
    if UB1 == 1:
        baseValues = np.random.normal(50, 5, (numBuyers + numSellers, numTokens)) 
    if UB2 == 1:
        agentDifferentiator = np.random.normal(0, 1, (numBuyers + numSellers, 1))
    if UB3 == 1:
        tokenDifferentiator = np.random.normal(0, 1, (1, numTokens)) 
    if UB4 == 1:
        buyerDifferentiator = np.random.normal(30, 1, (numBuyers, 1)) 
...
def generateState(agent, disclosure, depth):
    counters = [agent.periodSteps, agent.periodTrades]
    disclosureLength = len(disclosure)
    if (disclosureLength == 0) | (depth == 0):
        activityLog = []
    else:   
        if agent.gameSteps >= depth:
            disclosureCopy = deepcopy(disclosure)
            bidsDisclose, asksDisclose = False, False
            if 'bids' in disclosure:
                disclosureCopy.remove('bids')
                bidsDisclose = True
            if 'asks' in disclosure:
                disclosureCopy.remove('asks')
                asksDisclose = True
            
            activityLog = [[]]
            for i in range(1, depth+1):
                activityLog[0] += agent.df.iloc[-i][disclosureCopy].tolist()           
                if bidsDisclose:
            ...
            if 'asks' in disclosure:
                disclosureLength -= 1
                asksDisclose = True
            activityLog = [-1] * (disclosureLength*depth + 
                                  bidsDisclose*depth*agent.numBuyers+asksDisclose*depth*agent.numSellers)
...
class Log:
    ...
    def graphActions(self, agent, window_size = 100):
        memory = pd.DataFrame(agent.algo.memory, columns=['state', 'action', 'reward', 'newState', 'done'])
